...
class SensoroYunziBeacon {
  ...
  static parse(device) {
    ...
    let battery = sensorFrame.readUInt8(10);
    let luminosity = sensorFrame.readUInt16LE(14);
    let moveCount = sensorFrame.readUInt16LE(16);
    let isMoving = sensorFrame.readUInt8(18);

    let txPower, temperature;
    if (eddystoneFrame) {
      let eddystoneFrameType = eddystoneFrame.readUInt8(0);

      if (eddystoneFrameType === EDDYSTONE_FRAME_TYPE_UID) {
        txPower = eddystoneFrame.readInt8(1);
      }

      if (eddystoneFrameType === EDDYSTONE_FRAME_TYPE_TLM) {
        temperature = eddystoneFrame.readUInt16LE(4);
      }
    }
