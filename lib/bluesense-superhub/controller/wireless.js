...
class Controller {
  constructor() {
    ...
    this._bus.on(Message.route.connectWiFi, data => this._joinNetwork(data.payload.ssid, data.payload.passphrase));
    this._bus.on(Message.route.startWiFiScan, () => this._startWiFiScan());
    this._bus.on(Message.route.stopWiFiScan, () => this._stopWiFiScan());
    this._bus.on(Message.route.connectedToPlatform, () => this._onConnectedToPlatform());
  ...
  _wifiAppeared(network) {
    ...
    if (this._sendWiFiScanEvents) {
      this._bus.publish(new Message(Message.route.wifiUpdated, {networks: this._wifiNetworks}));
    }
  }
  ...
  _wifiError(message) {
    this._logger.info(`WiFi error: ${message}`);
    this._bus.publish(new Message(Message.route.wifiError, {errorMessage: message}));
  }

  _wifiJoined(network) {
    this._logger.info(`Joined network ${network.ssid}`);
    this._bus.publish(new Message(Message.route.wifiJoined, {network: network}));
  ...
  _wifiLeft() {
    this._bus.publish(new Message(Message.route.wifiLeft));
  }
  ...
  _joinNetwork(ssid, passphrase) {
    ...
    this._stopWiFiScan();
    this._logger.info(`Joining network ${ssid}...`);
  ...
  _onConnectedToPlatform() {
    this._bus.publish(
      this._wireless.network
        ? new Message(Message.route.wifiJoined, {network: this._wireless.network})
        : new Message(Message.route.wifiLeft)
    );
  }
}
