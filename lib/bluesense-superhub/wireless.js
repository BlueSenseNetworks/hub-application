...
class WirelessAdapter extends EventEmitter {
  ...
  static get errors() {
    return {
      ...
      unknownNetwork: 'The requested network does not exist'
    };
  }
  ...
  constructor(wireless) {
    ...
    this._wireless.on('appear', this.onAppear.bind(this));
    this._wireless.on('change', this.onChange.bind(this));
    this._wireless.on('signal', this.onChange.bind(this));
    this._wireless.on('vanish', this.onVanish.bind(this));
    this._wireless.on('error', this.onError.bind(this));
    this._wireless.on('join', this.onJoin.bind(this));
    this._wireless.on('leave', this.onLeave.bind(this));
    this._wireless.on('former', this.onFormer.bind(this));

    //the distro is set to automatically reload the wpa_supplicant daemon when this file changes, no further action needed
    this._wireless.commands.connect_wpa = 'sudo wpa_passphrase ":ESSID" ":PASSWORD" > /etc/wpa_supplicant/wpa_supplicant-wlan0.conf';

    //ref: https://wiki.netbsd.org/tutorials/how_to_use_wpa_supplicant/
    this._wireless.commands.connect_open = 'cat > /etc/wpa_supplicant/wpa_supplicant-wlan0.conf << EOF' +
      'network={' +
      '\tssid = ":ESSID"' +
      '\tkey_mgmt = NONE' +
      '\tpriority = 100' +
    ...
    this._wireless.commands.connect_wep = 'cat > /etc/wpa_supplicant/wpa_supplicant-wlan0.conf << EOF' +
      'network={' +
      '\tssid = ":ESSID"' +
      '\tkey_mgmt = NONE' +
      '\twep_key0 = ":PASSWORD"' +
  ...
  onFormer() {
    //An edge case is happening where we recognize that we are connected but we still don't have a SSID.
    //When this happens we don't get the join event. To circumvent this we set the connected flag to false and wait for
    //the join event to happen.
    this._wireless.connected = false;
  }
  ...
  join(network, passphrase) {
    return new Promise(resolve => this._wireless.join(WirelessAdapter.normalizedNetworkToInitial(network), passphrase, resolve));
  }
}
...
module.exports.create = () => {
  ...
  return new WirelessAdapter(wireless);
};
