...
class BleScanner extends EventEmitter {
  ...
  startScan() {
    this._logger.info(process.env.SUPERHUB_ROLE + ': BLE scan starting');
  ...
  stopScan() {
    ...
    this._logger.info(process.env.SUPERHUB_ROLE + ': BLE scan stopped');
...
//    this._logger.info(process.env.SUPERHUB_ROLE + ': BleScanner: ' + state + ' => ' + newState);
    state = newState;
  }
